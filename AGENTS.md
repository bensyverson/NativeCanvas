IMPORTANT: As you're implementing features, be sure to keep the [DocC documentation](./Sources/NativeCanvas/Documentation.docc/) up to date, updating [README](README.md) only if a new documentation file has been added, or if new users MUST know about your changes.

## Overview

You are working on NativeCanvas, a Swift library which allows users to draw into native CoreGraphics contexts (CGContext) with the familiar JavaScript Canvas API, via JavaScriptCore.
...
- [DocC documentation](./Sources/NativeCanvas/Documentation.docc/) - In-depth documentation about the NativeCanvas architecture, models and concepts
- [project/](project/) - Internal design documents and past project plans

API documentation is generated with DocC: `swift package generate-documentation --target NativeCanvas`. Ensure any new or updated code keeps our DocC annotations up to date with 100% coverage.
...
- IMPORTANT: In this project we ALWAYS follow strict "red/green" TDD; write tests for all example cases we need to handle and any new methods we're implementing, verify that they fail, and *then* proceed to implement your code changes. If a new test is green during the red stage, it's testing nothing and should be removed or refactored. If you must alter a previous test to get it to pass, explain exactly WHY to the user and get their consent.
- Before fixing a bug, try to create a regression test to catch it in the future.
- DO NOT begin a new chat by doing an extensive exploration of the entire codebase. That is wasteful, as this is a large codebase. Instead, read the README and use an Explore agent to read the DocC documentation if you want to get the lay of the land. Of course, once you have a specific need, you can explore as much of the code as you require.
...
- When implementing or updating SwiftUI components, use `Theme.swift`, which provides a lightweight design system.
- Always add #preview macros for new SwiftUI components
- Build all UI elements with accessibility and localization as first-class considerations from the very beginning. A component is not done until it is accessible and easily localizable.
- When necessary, create ViewModels to populate Views. Name ViewModels descriptively, and do not name the parameters that accept them `viewModel`.
- We generally inject cross-view "manager" objects via `.environment()`, keeping them in a "Coordinator" folder.
- Neser put more than one SwiftUI View in a file. Each View should be a separate file, and each View should have a working #preview.

## Application specific

  - Any Xcode projects use **folder-based source management** (no explicit file references in `project.pbxproj`). New Swift files are automatically discovered; there's no need to manually add them to the project.
- The Xcode test targets also auto-discover test files.
- **The app module defaults every declaration to `@MainActor`.** Never add `@MainActor` to new types — it's already implicit. Only mark something `nonisolated` when a protocol requires it (e.g. `fileWrapper`, `readableContentTypes`) or when a method does blocking I/O or processing that would stall the UI thread. Do not use `@unchecked Sendable` unless approved by the user.
- CAREFUL: Running `xcodebuild test` from the command-line is slow. Rather than run it repeatedly to grep different things, run it *once* and pipe the result into a tmp file. Then you can grep the tmp file.
- Prior to commiting changes, you must run the linter (`swiftformat . --lint`) and re-run to format if necessary (`swiftformat .`), or fix any issues manually if necessary. You must also run the full test suite. Linting and testing are both pre-commit hooks, so this will allow you to catch problems before attempting a commit.
- IMPORTANT: If you are running within Xcode, remember to use your Xcode-specific tools!
